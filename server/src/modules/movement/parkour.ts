...
type JumpState = {
  pos: { x: number; y: number; z: number; clone?: () => { x: number; y: number; z: number } };
  vel: { x: number; y: number; z: number };
  yaw: number;
  onGround: boolean;
  isInLava?: boolean;
  control: { forward: boolean; sprint: boolean; jump: boolean; sneak: boolean; back: boolean; left: boolean; right: boolean };
};

function newJumpState(bot: Bot, x: number, y: number, z: number, yaw: number): JumpState | null {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { PlayerState } = require("prismarine-physics") as {
      PlayerState: new (b: Bot, c: JumpState["control"]) => JumpState;
    };
    const control: JumpState["control"] = {
      forward: true,
      back: false,
      left: false,
      right: false,
...
function simDistToEdge(bot: Bot, x: number, z: number, y: number, ux: number, uz: number): number {
  const py = Math.floor(y);
  for (let t = 0.05; t <= 3.2; t += 0.1) {
    if (standYAt(bot, Math.floor(x + ux * t), py, Math.floor(z + uz * t)) === null) return t;
  }
  return 3.2;
}

/**
 * Dry-run the jump (sprint + edge jump). Instant — does not move the bot.
 * Returns the takeoff that lands, including a side step if the current lip falls.
 */
function predictGapJump(
  bot: Bot,
  landing: { x: number; y: number; z: number },
  gap: number
): { x: number; y: number; z: number } | null {
  if (!bot.entity) return null;
  const physics = bot.physics as { simulatePlayer?: (state: unknown, world: unknown) => void };
  if (!physics.simulatePlayer) {
  ...
  const py = pos.y;
  const world = { getBlock: (p: { x: number; y: number; z: number }) => bot.blockAt(p as never, false) };
  const jumpAt = gap >= 4 ? 0.62 : 0.5;

  const candidates: Array<{ x: number; y: number; z: number }> = [{ x: pos.x, y: py, z: pos.z }];
  for (const side of [0.35, -0.35, 0.55, -0.55]) {
    const x = pos.x + px * side;
    const z = pos.z + pz * side;
    if (standYAt(bot, Math.floor(x), Math.floor(py), Math.floor(z)) != null) candidates.push({ x, y: py, z });
  }
  ...
  for (const c of candidates) {
    const state = newJumpState(bot, c.x, c.y, c.z, yaw);
    if (!state) continue;
    let airborne = false;
    for (let t = 0; t < ticks; t++) {
      const edge = simDistToEdge(bot, state.pos.x, state.pos.z, state.pos.y, ux, uz);
      state.control.forward = true;
      state.control.sprint = true;
      state.control.sneak = false;
      state.control.jump = Boolean(state.onGround && edge <= jumpAt);
      if (state.control.jump) airborne = true;
      try {
      ...
      const d = Math.hypot(state.pos.x - lx, state.pos.z - lz);
      if (airborne && state.onGround && t > 3) {
        const ok = d < 1.55 && Math.abs(state.pos.y - ly) < 1.7;
        const fell = state.pos.y < ly - 1.3;
        if (ok) return c;
        if (!fell && (!best || d < best.dist)) best = { ...c, dist: d };
        break;
      }
    }
  }
  // No predicted landing — refuse the jump (walk the ledge instead).
  return best && best.dist < 1.2 ? { x: best.x, y: best.y, z: best.z } : null;
}
...
export async function executeGapJump(
  ...
  try {
    ...
    const takeoff = predictGapJump(bot, landing, g);
    if (!takeoff) {
      instance.getLogger().info("Jump predicted miss", `gap=${g} — walking instead`);
      return false;
    }
    const off = Math.hypot(takeoff.x - pos0.x, takeoff.z - pos0.z);
    if (off > 0.2) {
      try {
        await alignManualLookAt(bot, v3(takeoff.x, takeoff.y + 0.4, takeoff.z));
        bot.setControlState("sneak", true);
        bot.setControlState("forward", true);
        bot.setControlState("sprint", false);
        const tSide = Date.now();
        while (Date.now() - tSide < 380 && bot.entity && !token.cancelled) {
          if (Math.hypot(bot.entity.position.x - takeoff.x, bot.entity.position.z - takeoff.z) < 0.16) break;
          await sleep(20);
        }
        clearControls(bot);
        await sleep(40);
