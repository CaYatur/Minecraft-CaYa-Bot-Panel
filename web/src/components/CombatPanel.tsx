...
/**
 * Faz 6 — Dövüş kartı. Tasarım: InventoryPanel / TasksPanel ile aynı koyu kart dili.
 * Gerçekçilik ayarları BotConfig.combat altında (RealismLayer kapatılamaz, sadece değerler).
 */
    ...
    <div className="flex h-full flex-col gap-4 overflow-y-auto">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
          <div className="mb-2 text-[10px] font-semibold tracking-wide text-zinc-500 uppercase">Durum</div>
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <span
              className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                combat.fighting ? "bg-red-500/15 text-red-300" : "bg-zinc-800 text-zinc-400"
              }`}
            >
              {MODE_TR[combat.mode]}
            </span>
            ...
            {!combat.activeTarget && <span className="text-xs text-zinc-600 italic">Aktif hedef yok</span>}
          </div>
          <p className="mt-2 text-[11px] leading-relaxed text-zinc-500">
            RealismLayer her zaman açık: bak → menzil ≤ {cfg.reach} · görüş hattı · vuruş temposu · yumuşak dönüş · tepki{" "}
            {cfg.reactionMsMin}–{cfg.reactionMsMax} ms. Ham aimbot/duvar arkası vuruş yok (§9).
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              onClick={() => act({ type: "stop-combat" }, "Dövüş bırakıldı")}
              className="rounded-lg bg-zinc-800 px-3 py-1.5 text-xs font-medium text-red-300 hover:bg-zinc-700"
            >
              ■ Dövüşü Bırak
            </button>
            <button
              ...
              disabled={!online}
              className="rounded-lg bg-zinc-800 px-3 py-1.5 text-xs font-medium text-amber-300 hover:bg-zinc-700 disabled:opacity-40"
            >
              Kaç
            </button>
          </div>
        ...
        {/* savunma */}
        <div className="rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
          <div className="mb-2 text-[10px] font-semibold tracking-wide text-zinc-500 uppercase">Savunma modu</div>
          ...
          <p className="mt-2 text-[11px] text-zinc-500">
            Can ≤ {cfg.fleeAtHealth} olunca dövüş bırakılır, uzaklaşılır (İ6). Kovalama: {cfg.chaseDistance} blok.
          </p>
        </div>
      </div>
      ...
      {/* saldırı */}
      <div className="rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
        <div className="mb-2 text-[10px] font-semibold tracking-wide text-zinc-500 uppercase">Hedefli saldırı</div>
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="w-44 rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-sm text-zinc-100 outline-none focus:border-indigo-500"
          />
          <button
            onClick={() => act({ type: "attack", player: target }, `Saldırı: ${target}`)}
            disabled={!online || !target.trim()}
            className="rounded-lg bg-red-900/50 px-3 py-1.5 text-sm font-medium text-red-200 hover:bg-red-800/50 disabled:opacity-40"
          >
            ⚔ Saldır
          </button>
          <input
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
        <div className="mb-2 text-[10px] font-semibold tracking-wide text-zinc-500 uppercase">Gerçekçilik ayarları</div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <NumField
            ...
            min={1}
            max={4}
            onCommit={(v) => void patchCombat({ reach: v })}
          />
          <NumField label="CPS tavanı (1.8)" value={cfg.cpsCap} min={1} max={15} onCommit={(v) => void patchCombat({ cpsCap: v })} />
          <NumField
            label="Tepki min (ms)"
            value={cfg.reactionMsMin}
            min={0}
            max={1000}
            onCommit={(v) => void patchCombat({ reactionMsMin: v })}
          />
          <NumField
            label="Tepki max (ms)"
            value={cfg.reactionMsMax}
            min={0}
            max={1500}
            onCommit={(v) => void patchCombat({ reactionMsMax: v })}
          />
          <NumField
            label="Dönüş °/tick"
            value={cfg.turnSpeedDegPerTick}
            min={5}
            max={90}
            onCommit={(v) => void patchCombat({ turnSpeedDegPerTick: v })}
          />
          <NumField
          ...
          <label className="flex items-center gap-2 text-sm text-zinc-300">
            <input
              type="checkbox"
              checked={cfg.jumpCrit}
              onChange={(e) => void patchCombat({ jumpCrit: e.target.checked })}
            />
      ...
      {/* ölüm / loot */}
      <div className="rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
        <div className="mb-2 text-[10px] font-semibold tracking-wide text-zinc-500 uppercase">Ölüm &amp; loot</div>
        {combat.lastDeath ? (
          <div className="flex flex-wrap items-center gap-3 text-sm">
            <span className="mono text-zinc-300">
              📍 {fmtPos(combat.lastDeath)}{" "}
              <span className="text-zinc-600">({combat.lastDeath.dimension})</span>
            </span>
            <span className={`text-xs ${lootLeft > 0 ? "text-amber-300" : "text-zinc-600"}`}>
              {lootLeft > 0 ? `Loot süresi: ${Math.floor(lootSec / 60)}:${String(lootSec % 60).padStart(2, "0")}` : "Süre dolmuş olabilir"}
            </span>
            <button
              onClick={() => act({ type: "loot-death" }, "Ölüm noktasına gidiliyor")}
              disabled={!online || lootLeft <= 0}
              className="rounded-lg bg-indigo-600/80 px-3 py-1.5 text-xs font-medium text-white hover:bg-indigo-500 disabled:opacity-40"
            >
              Eşyaları Geri Topla
            </button>
          </div>
