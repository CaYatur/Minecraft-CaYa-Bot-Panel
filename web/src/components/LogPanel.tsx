...
const LEVEL_STYLE: Record<LogLevel, { label: string; row: string; chip: string }> = {
  debug: { label: "DEBUG", row: "text-zinc-500", chip: "bg-zinc-800 text-zinc-400" },
  info: { label: "BİLGİ", row: "text-sky-300", chip: "bg-sky-500/10 text-sky-300" },
  success: { label: "BAŞARI", row: "text-emerald-300", chip: "bg-emerald-500/10 text-emerald-300" },
  warn: { label: "UYARI", row: "text-amber-300", chip: "bg-amber-500/10 text-amber-300" },
  error: { label: "HATA", row: "text-red-300", chip: "bg-red-500/10 text-red-300" }
};
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-1.5 border-b border-zinc-800 pb-2">
        ...
        <span className="ml-auto text-[11px] text-zinc-600">{visible.length} kayıt</span>
      </div>
      ...
      >
        {visible.length === 0 && <div className="py-8 text-center text-zinc-600">Log kaydı yok</div>}
        {visible.map((l, i) => (
          <div key={i} className={`flex gap-2 rounded px-1 leading-relaxed hover:bg-zinc-900/60 ${LEVEL_STYLE[l.level].row}`}>
            <span className="shrink-0 text-[10px] text-zinc-600 tabular-nums">{fmtTime(l.ts)}</span>
